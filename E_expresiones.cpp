	...
	float evaluar(string s, map<string, vector<string> >tabla_var){
		...
		stack<float>pila;	//aceptará también enteros?
	...
	int main(){
		...
		tabla_var["$j"].push_back("1");
		
		//Simulaci�n de tokens
		vector <string>v;
		/*
		v.push_back("$j");
		v.push_back("+");
		...
		cout<<e_expresiones(v,tabla_var);
		
		/*
		int res = e_expresiones(v,tabla_var);
		cout<<"Respuesta"<<res<<"\n";
		if(res){
			cout<<"Expresion evaluada";
		}*/
	}
